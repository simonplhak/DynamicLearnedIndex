...
logger.info(f'Experiment ID: {experiment_id}')
logger.info(experiment_config)
logger.info(f'Build time: {time_fmt(build_result.time)}')
logger.info(f'Insert throughput: {int(len(X) / build_result.time)} IPS')  # TODO: store persistently?
logger.info(f'Total model training time: {time_fmt(build_result.total_model_training_time())}')
logger.info(f'Framework overhead time: {time_fmt(build_result.time - build_result.total_model_training_time())}')
